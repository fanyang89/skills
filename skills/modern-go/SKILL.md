---
name: modern-go
description: Apply modern Go syntax guidelines based on project's Go version. Use when user asks for modern Go code guidelines.
---
...
1. Detect the Go version by searching for `go.mod` files in the repository
2. If version detected:
   - Inform the user which Go version was detected
   - Apply all modern Go features up to and including that version
   - Do NOT use features from newer Go versions
3. If version cannot be detected:
   - Ask the user which Go version to target
   - Default options: 1.23, 1.24, 1.25, 1.26
